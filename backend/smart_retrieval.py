...
class SmartRetriever:
    ...
    def search_by_document(
        ...
        memories = self.db.query(Memory).filter(
            Memory.user_id == self.user_id,
            Memory.source_file == document_name
        ).all()
        ...
        for memory in memories:
            score = self._calculate_enhanced_score(
                memory, 0.5, query, 'document_query', None
            )
            scored.append({
                'memory': memory,
                'score': score,
    ...
    def get_related_memories(
        ...
        """
        target_memory = self.db.query(Memory).filter(
            Memory.id == memory_id,
            Memory.user_id == self.user_id
        ).first()
    ...
    def retrieve_with_document_priority(
        ...
        if has_docs:
            ...
            results = self.smart_retriever.retrieve_with_ranking(
                query, top_k=top_k, intent=intent,
            ...
            if len(doc_results) < top_k // 2:
                ...
                additional = self.smart_retriever.retrieve_with_ranking(
                    query, top_k=top_k * 2, intent='document_query',
                    conversation_context=conversation_context
                )
