...
class PersonalityEngine:
    ...
    @classmethod
    def get_system_prompt(cls, language: str = 'en', context: Optional[Dict] = None) -> str:
        """
...
- Prioritize user privacy above all else"""
    ...
    @classmethod
    def get_response_style(cls, intent: str, user_emotion: Optional[str] = None) -> Dict:
        """
    ...
    def should_be_concise(cls, question: str) -> bool:
        ...
        if any(question_lower.startswith(ind) for ind in brief_indicators):
            return True
    ...
    def should_be_detailed(cls, question: str) -> bool:
        ...
        detail_indicators = [
            'explain', 'describe', 'how does', 'why does', 'what is the',
            'tell me about', 'elaborate', 'expand', 'detail', 'comprehensive'
        ]
        
        return any(indicator in question_lower for indicator in detail_indicators)
    
    @classmethod
    def detect_emotion(cls, message: str) -> Optional[str]:
        """
        Detect user emotion from their message.
        """
        message_lower = message.lower()
        
        frustration_indicators = ['frustrated', 'annoyed', 'angry', 'upset', 'not working', 'broken']
        confusion_indicators = ['confused', 'don\'t understand', 'unclear', 'what do you mean', 'help']
